...
class Host(object):
    ...
    def _create_cloud_init_image(self):
        ...
        write_files_provide_network_scripts = False
        for e in user_data['write_files']:
            if e['path'].startswith('/etc/sysconfig/network-scripts/ifcfg-'):
                logging.debug("Network already defined in cloud_init "
                              "write_files section. We reuse the "
                              "network-scripts settings for node %s." %
                              (self.hostname))
                write_files_provide_network_scripts = True
                break

        if not write_files_provide_network_scripts:
